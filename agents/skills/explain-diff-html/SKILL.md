...
- **Use `render.py` from this skill directory instead of hand-writing HTML.**
  ...
  ```
  python3 <skill-directory>/render.py <spec.json>
  ```
  ...
  content spec, not the full HTML page. Run
  `python3 <skill-directory>/render.py --help` for the exact JSON schema if you haven't
  used it recently. Resolve `<skill-directory>` from the directory containing this
  `SKILL.md`; do not copy the renderer elsewhere.
